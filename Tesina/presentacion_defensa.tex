...
\begin{frame} {Correo No Deseado (Spam)}
  \begin{itemize}
    ...
    \item \textbf{El Gran Desafío:} Entrenar estos modelos presenta un problema debido a los correos: el desbalanceo entre los legítimos y los de \textit{spam}.
  \end{itemize}
\end{frame}
...
\begin{frame}{Uso de StandardScaler}
  \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Ajustado exclusivamente sobre datos de entrenamiento para evitar \textit{Data Leakage}.
...
\begin{frame}{Técnica de sobremuestreo: SMOTE}
  \textbf{\textit{Synthetic Minority Over-sampling Technique}}
  \begin{itemize}
    \item Algoritmo estándar en la industria para la creación de datos sintéticos.
    \item Analiza instancias de la clase minoritaria (\textit{spam}) y busca a sus $k$ vecinos más cercanos ($k=5$).
...
\begin{frame}{Técnica de sobremuestreo: ADASYN}
  ...
  \begin{itemize}
    ...
    \item Calcula la densidad de probabilidad para definir qué tan ``difícil" de aprender es cada instancia de \textit{spam}.
...
\begin{frame}{Modelos de Clasificación: Parámetros}
  ...
  \vspace{0.3cm}
  \begin{center}
      \scriptsize \textit{* Se utilizaron los parámetros por defecto de} \texttt{scikit-learn}\textit{, únicamente cambiando manualmente el umbral de decisión del SVM a \textbf{0,2}.}
  \end{center}
\end{frame}
...
\begin{frame}{Justificación de la proporción óptima}
  ...
  \vspace{0.2cm}
  La relación 70-30 aumenta el F1-\textit{Score} sin sacrificar el \textit{Recall}.
\end{frame}
...
%  \vspace{-0.5cm}
  \begin{itemize}\scriptsize
    \item \textbf{Random Forest (G-SMOTE):} Alcanzó la mayor \textit{Precision} (\textbf{0,84}) y el mejor F1-\textit{Score} (\textbf{0,80}).
    \item \textbf{SVM (ADASYN):} Logró el mayor \textit{Recall}, con un valor de \textbf{0,86}.
  \end{itemize}
\end{frame}
...
\vspace{-0.4cm}
  ...
  \begin{itemize}\scriptsize
    \item Al aplicar ADASYN (70-30), el modelo eleva su \textit{Recall} a 0,86.
    \item Se aproxima fuertemente al rendimiento del modelo entrenado con el \textit{dataset} original (\textit{Recall} de 0,89), superando ampliamente al entrenamiento con el conjunto desbalanceado.
  \end{itemize}
\end{frame}

\section{Conclusiones}

\begin{frame}{Conclusiones: Mejores resultados}
  No existe un sobremuestreo perfecto; el ``ganador'' depende de la prioridad buscada.
  \vspace{0.4cm}
  
  \textbf{Prioridad: Minimizar Falsos Positivos} 
  \begin{itemize}
      \item \textbf{Ganador:} \textit{Random Forest} + G-SMOTE
      \item Gracias a la restricción esférica, alcanzó la mejor \textit{Precision} (0,84) y el mejor F1-\textit{Score} (0,80) del estudio.
  \end{itemize}

  \vspace{0.3cm}
  \textbf{Prioridad: Máxima Detección} 
  \begin{itemize}
      \item \textbf{Ganador:} SVM (Umbral ajustado a 0.2) + ADASYN
      \item Enfoque si se prioriza detectar amenazas. Logró un  \textit{Recall} de 0,86, detectando casi todas las amenazas a costo de permitir más falsos positivos.
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusiones: Sobremuestreo vs. datos reales}
  \begin{itemize}
    ...
    Se evaluó el método de sobremuestreo ganador contra el conjunto \textit{SpamBase} original el cual poseía naturalmente un 40\% de \textit{spam}.
    \vspace{0.2cm}
    \item \textbf{Original vs Sobremuestreado:} 
    Los datos originales demostraron una ligera superioridad (\textit{Recall} de 0,89). 
    \\
     Sin embargo, el modelo entrenado con ADASYN logró un \textit{Recall} de 0,86 partiendo desde un desbalance del 5\%. 
    \vspace{0.2cm}
