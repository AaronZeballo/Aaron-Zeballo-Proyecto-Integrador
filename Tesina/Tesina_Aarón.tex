...
Los modelos de aprendizaje automático se enfrentan al desequilibrio de clases; este desequilibrio perjudica el aprendizaje de los algoritmos, los cuales tienden a ignorar la clase minoritaria y pierden capacidad predictiva. Para tratar de solucionar esta problemática, el presente trabajo se centró en el estudio de distintos algoritmos de sobremuestreo de datos como solución para equilibrar las clases. Se evaluaron técnicas clásicas como SMOTE y ADASYN, así como sus evoluciones modernas, poniéndolas a prueba con una problemática importante en la ciberseguridad: la detección de correos no deseados (\textit{spam}). Para medir su efectividad se utilizaron tres clasificadores: Árbol de Decisión, \textit{Random Forest} y Máquinas de Soporte Vectorial (SVM).

Los resultados mostraron un buen desempeño por parte de las técnicas de sobremuestreo para contrarestar el desbalanceo en las clases. Específicamente, el método ADASYN combinado con SVM tuvo el mejor rendimiento a la hora de detectar \textit{spam}, logrando alcanzar un \textit{Recall} de 0.86 y un \textit{F1-Score} de 0.75 sobre la clase minoritaria. Esto demostró que, a partir de un conjunto de datos fuertemente desbalanceado y posteriormente sobremuestreado, es posible obtener resultados predictivos muy cercanos a los que se pueden obtener a partir de un conjunto de datos con mayor equilibrio entre clases y ampliamente superior a los de un conjunto de datos desbalanceado.
...
\label{eq:rf_voting}
\end{equation}
donde $A$ es la cantidad total de árboles en el bosque y $\mathbb{I}$ es una función indicadora que suma un voto ($1$) cada vez que un árbol $a$ vota por una clase en particular $c$. Este enfoque combinado permite compensar los errores individuales y logra un modelo predictivo más estable.
...
Tras las pruebas realizadas, se concluye que no existe una técnica de sobremuestreo perfecta sino que depende de lo que se priorice:

Si el objetivo principal es garantizar que los correos legítimos no sean bloqueados accidentalmente, la combinación de \textit{Random Forest} con el método G-SMOTE demostró ser la más efectiva. Esta configuración logró el \textit{F1-Score} más alto del estudio ($0.80$) y una excelente precisión, minimizando la pérdida de correos importantes para el usuario.

Cuando se quiere detectar la mayor cantidad de correos maliciosos posibles a cambio de algún falso positivo, ADASYN combinado con Máquinas de Soporte Vectorial (SVM) es la combinación deseada. Este enfoque alcanzó un \textit{Recall} de $0.86$, identificando la inmensa mayoría de las amenazas.

Finalmente, la comparación contra un conjunto de datos compuesto por instancias reales en su totalidad demostró buenos resultados por parte del conjunto sobremuestreado. Las técnicas aplicadas, especialmente ADASYN, permitieron que un modelo que en un principio estaba fuertemente desbalanceado recuperara en gran medida su capacidad de detectar \textit{spam}, acercando su rendimiento al de un modelo entrenado con datos originales abundantes. Esto muestra que las técnicas de sobremuestreo son una herramienta robusta y eficaz a la hora de trabajar con conjuntos de datos con cantidad de ejemplos dispares entre sus clases.
